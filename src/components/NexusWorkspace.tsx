...
import { Search, Loader2, Globe, ArrowUpRight } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-[#111619] text-[#A4ACA6] overflow-hidden">
      <header className="flex-shrink-0 h-16 border-b border-[#2F3A3E] px-6 flex items-center justify-between z-10 bg-[#111619]/90 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <Search size={18} className="text-[#D89A3A]" />
          <h2 className="text-[#CBD3CC] font-serif tracking-wide text-sm">Nexus Grounding</h2>
        </div>
        <div className="text-[10px] uppercase tracking-[0.2em] text-[#4F5A58]">gemini-3.5-flash + Google Search</div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-12 flex flex-col items-center">
        <div className="w-full max-w-3xl space-y-8">
          ...
          <form onSubmit={handleSearch} className="relative group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-[#4F5A58] group-focus-within:text-[#D89A3A] transition-colors" size={20} />
            <input
              type="text"
              value={prompt}
